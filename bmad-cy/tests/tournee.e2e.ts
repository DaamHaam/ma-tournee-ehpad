...
test('sauvegarde complète puis restauration sur un appareil vierge', async ({ browser, page }) => {
  ...
  await page.getByRole('button', { name: /^Note pour Petit Jeanne/ }).click()
  await page.getByRole('textbox', { name: 'Note du jour pour Petit Jeanne' }).fill('sauvegardée')
  await page.getByRole('link', { name: 'Patients' }).click()
  await page.getByRole('link', { name: /Robert Paul/ }).click()
  ...
  fresh.once('dialog', dialog => { expect(dialog.message()).toContain('4 patients, 1 journées'); void dialog.accept() })
  await fresh.getByLabel('Fichier de sauvegarde').setInputFiles(file)
  await expect(fresh.getByText('Sauvegarde restaurée.')).toBeVisible()
  await fresh.goto('/#/?date=2026-09-21')
  await expect(fresh.getByRole('button', { name: 'B pour Petit Jeanne' })).toHaveAttribute('aria-pressed', 'true')
  await expect(fresh.getByRole('textbox', { name: 'Note du jour pour Petit Jeanne' })).toHaveValue('sauvegardée')
  await fresh.goto('/#/patients')
  await expect(fresh.getByText('Robert Paul')).toHaveCount(0)
  await fresh.getByLabel('Archivés').check()
